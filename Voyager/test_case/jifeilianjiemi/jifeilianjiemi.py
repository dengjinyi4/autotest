...
def huoqulianjie(mydriver):
    time.sleep(3)
    mydriver.get('https://172.16.17.213/mgrGetCustomAdLinkCodeEdit.do?campaignId=139')
    # select=mydriver.find_element_by_id('selfSelect')
    # Select(select).select_by_value(3)
    mydriver.find_element_by_xpath('//*[@id="selfSelect"]/option[3]').click()
    mydriver.find_element_by_id('webSiteId').send_keys('871361')
